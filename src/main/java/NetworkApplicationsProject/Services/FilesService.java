...
public class FilesService {
    ...
    public String deleteFile(Integer fileId) {
        ...
        if (targetFile.isPresent()) {
            if (HandleCurrentUserSession.getCurrentUser().getId().equals(targetFile.get().getOwner().getId())) {
    ...
    public List<FileModel> getUploadUserFiles(Integer groupId) {
        Optional<GroupModel> targetGroup = groupRepository.findById(groupId);
        if (targetGroup.isPresent()) {
            if (checkIsCurrentUserInGroup(targetGroup.get())) {
                return fileRepository.findByOwnerId(HandleCurrentUserSession.getCurrentUser().getId());
            } else {
                throw new CustomException("you are not member in this group", HttpStatus.FORBIDDEN);
            }
        } else {
